...
<div class="card">
	...
	<div class="card-body">
		<form action="{{Route('produk.best_seller.update',$best_seller->id)}}" method="POST" enctype="multipart/form-data" id="form_menu">
			{{csrf_field()}}
			<div class="row">
				<div class="col-lg-12">
					...
					<select class="form-control" name="best_seller" required="">
						<?php foreach ($produk as $key => $value) { ?>
							<option value="{{ $value->id }}" <?php if($best_seller->product_id == $value->id){ ?> selected <?php } ?> >{{ $value->getProdukLanguage->judul }}</option>
						<?php } ?>
					</select>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-12">
					...
					<div class="kt-avatar kt-avatar--outline" id="kt_user_add_avatar" style="width: 100%;">
						<div class="kt-avatar__holder" style="width: 100%; background-image: url('{{-- asset('assets/admin/assets/media/icons/')  --}}/{{--$best_seller->icon--}}')"></div>
