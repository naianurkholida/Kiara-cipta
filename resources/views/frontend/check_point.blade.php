@extends('frontend.component.master')

@section('content')
<div class="section nobg nobottommargin clearfix" style="margin-top: 0;">
    <div class="container clearfix">
        <div class="heading-block center noborder" data-heading="O">
            <h3 class="nott ls0">Check Point</h3>
        </div>
    </div>
    <div class="container">
        <div class="container">
            <iframe style="border:0px #ffffff none;" width="100%" height="400" src="http://103.11.134.45:1717/apex/f?p=889:1" scrolling="no" frameborder="1" allowfullscreen=""></iframe>
            <!-- <iframe src="http://localhost:8000/derma-express/checkpoint" style="border:0px #ffffff none;" name="myiFrame" scrolling="no" frameborder="1" marginheight="0px" marginwidth="0px" height="400px" width="600px" allowfullscreen></iframe> -->
        </div>
    </div>

</div>
@endsection